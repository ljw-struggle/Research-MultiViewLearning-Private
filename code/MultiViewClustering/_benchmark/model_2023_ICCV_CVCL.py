...
def validation(model, dataset, view, data_size):
    model.eval()
...
def benchmark_2023_ICCV_CVCL(dataset_name, learning_rate, weight_decay, batch_size, seed, mse_epochs, con_epochs, normalized, temperature_l, dim_high_feature, dim_low_feature, hidden_dims, alpha, beta):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for epoch in range(mse_epochs):
        ...
        # print('Pre-training epoch {}, Loss:{:.7f}'.format(epoch, np.mean(losses)))
    ...
    for epoch in range(con_epochs):
        ...
        for batch_idx, (x_list, _, _) in enumerate(dataloader):
            ...
            optimizer.step()
            losses.append(loss.item())
        # print('Contrastive_train epoch {}, Loss:{:.7f}'.format(epoch, np.mean(losses)))
    acc, nmi, pur, ari = validation(model, dataset, view, data_size)
    return acc, nmi, pur, ari
...
if __name__ == "__main__":
    ...
    acc, nmi, pur, ari = benchmark_2023_ICCV_CVCL(dataset_name, learning_rate, weight_decay, batch_size, seed, mse_epochs, con_epochs, normalized, temperature_l, dim_high_feature, dim_low_feature, hidden_dims, alpha, beta)
