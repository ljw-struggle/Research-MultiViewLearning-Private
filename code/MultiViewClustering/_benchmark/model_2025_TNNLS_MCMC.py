...
class MCMCLoss(nn.Module):
    ...
    def compute_cluster_loss(self, q_centers, k_centers): # MCMC has a error implementation here. (maybe hahaha)
        ...
        neg = q_centers.mm(q_centers.T) / 1.0 # shape: (class_num, class_num)
        pos = (q_centers * k_centers).sum(dim=1) / 1.0
        neg[torch.arange(self.class_num), torch.arange(self.class_num)] = pos
        loss = - (pos - torch.logsumexp(neg, dim=1)) # InfoNCE loss
    ...
    def compute_centers(self, h, pseudo_label): 
        ...
        if len(pseudo_label.size()) > 1:
            ...
            weight = F.one_hot(pseudo_label, num_classes=self.class_num).T.to(self.device) # shape: (class_num, batch_size)
...
def benchmark_2025_TNNLS_MCMC(dataset_name, batch_size, temperature_f, temperature_l, learning_rate, weight_decay, mse_epochs, con_epochs, feature_dim, high_feature_dim, seed):
    ...
    nmi, ari, acc, pur = validation(model, dataset, view, data_size, class_num)
...
if __name__ == "__main__":
    ...
    acc, nmi, pur, ari = benchmark_2025_TNNLS_MCMC(dataset_name, batch_size, temperature_f, temperature_l, learning_rate, weight_decay, mse_epochs, con_epochs, feature_dim, high_feature_dim, seed)
