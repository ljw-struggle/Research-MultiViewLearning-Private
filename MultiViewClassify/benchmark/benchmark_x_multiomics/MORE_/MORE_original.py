...
def load_feature_and_hyperedge(data_dir, view_list):
    ...
    return data_train_list, data_test_list, label, data_train_indices, data_test_indices, adj_train, adj_test, sample_weight
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, input_data, mask=None): # input_data: (bs, modal_num, input_data_dims)
        ...
        attn = torch.matmul(q / math.sqrt(k.size(-1)), k.transpose(2, 3)) # shape: (bs, n_head, modal_num, modal_num)
        attn = attn.masked_fill(mask == 0, -1e9) if mask is not None else attn # shape: (bs, n_head, modal_num, modal_num)
        attn = attn / abs(attn.min())
        attn = F.softmax(F.normalize(attn, p=1, dim=-1), dim=-1) # shape: (bs, n_head, modal_num, modal_num)
...
if __name__ == "__main__":    
    ...
    parser.add_argument('--data_folder', type=str, default='BRCA', help='Data folder.')
    ...
    num_class_dict = {'BRCA': 5, 'ROSMAP': 2, 'GBM': 4}
    data_folder = args.data_folder; view_list = [1, 2, 3]; num_epoch_pretrain = 500; num_epoch = 1500; lr_e_pretrain = 1e-3; lr_e = 5e-4; lr_c = 1e-3; num_class = num_class_dict[data_folder]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    data_train_list, data_test_list, label, data_train_indices, data_test_indices, adj_train, adj_test, sample_weight_train = load_feature_and_hyperedge(data_folder, view_list)
    model_dict = {}
    ...
    model_dict["C"] = TransformerEncoder(input_data_dim=[10, 10, 10], n_hidden=args.n_hidden, n_head=args.n_head, dropout=args.dropout, num_class=num_class).to(device)
    ...
    for epoch in range(num_epoch+1):
        ...
        if epoch % 50 == 0:
            ...
            if num_class == 2:
                print("Test ACC: {:.3f}".format(accuracy_score(label[data_test_indices], prob.argmax(1))))
                print("Test F1: {:.3f}".format(f1_score(label[data_test_indices], prob.argmax(1))))
                print("Test AUC: {:.3f}".format(roc_auc_score(label[data_test_indices], prob[:,1])))
            else:
                print("Test ACC: {:.3f}".format(accuracy_score(label[data_test_indices], prob.argmax(1))))
                print("Test F1 weighted: {:.3f}".format(f1_score(label[data_test_indices], prob.argmax(1), average='weighted')))
                print("Test F1 macro: {:.3f}".format(f1_score(label[data_test_indices], prob.argmax(1), average='macro')))
