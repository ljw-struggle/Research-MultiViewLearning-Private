...
class HF(nn.Module):
    ...
    def forward(self, data_list, G):
        ...
        MMfeature_hypergraph = self.HGNN(MMfeature, G) # shape: N X hidden_dim[0]
        
        MMlogit = self.MMClasifier(MMfeature_hypergraph) # shape: N X num_class
        # MMlogit = self.MMClasifier(torch.cat([MMfeature, MMfeature_hypergraph], dim=1)) # shape: N X num_class
        return MMlogit, FeatureInfo, TCPLogit, TCPConfidence, ModalityEmbedding, MMfeature, MMfeature_hypergraph
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_dir', default='./result/data_MOGONET/HF/BRCA/', help='The output dir.')
    ...
    data_list, label, data_train_indices, data_test_indices, hyperedge_multi_omics, pre_calc_G = load_feature_and_hyperedge(args.data_dir, use_mRNA=True, use_meth=True, use_miRNA=True, k_list=[k], is_prob=True, m_prob=1, edge_type='euclid')
    ...
    model = HF(dim_list, hidden_dim, num_class, dropout=0.5).to(device)
    ...
    if args.verbose == 2:
        ...
        flops, params = profile(model, inputs=(data_list, pre_calc_G), verbose=False, custom_ops={HGNN_conv: count_hgnn_flops}) # return MACs and params; flops = 2 * MACs
