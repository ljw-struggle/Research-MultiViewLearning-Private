...
class MMD(nn.Module):
    ...
    def forward(self, data_list):
        ...
        Q = torch.stack([Feature[view] for view in range(self.views)], dim=1) # shape: (batch_size, views, hidden_dim[0])
        ...
        output = self.mha(Q, K, V)[0] # shape: (batch_size, views, hidden_dim[0])
        MMfeature = output.contiguous().view(-1, output.shape[1] * output.shape[2]) # shape: (batch_size, views*hidden_dim[0])
