        ...
        if edge_type == 'euclid':
            ...
            for center_idx in range(distance_mat.shape[0]):
                ...
                for node_idx in nearest_idx[:k]:
                    hyperedge_mat[node_idx, center_idx] = np.exp(-distance_vec[node_idx] ** 2 / (m_prob * distance_vec_avg) ** 2) if is_prob else 1.0 # Gaussian kernel for computing the hyperedge weight (Transformation of euclidean distance to probability)
        elif edge_type == 'pearson':
            distance_mat = pd.DataFrame(X.T).corr(method='pearson').to_numpy() # between -1 and 1
            hyperedge_mat = np.zeros(distance_mat.shape)
            for center_idx in range(distance_mat.shape[0]):
