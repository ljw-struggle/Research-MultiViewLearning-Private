...
def load_feature_and_hyperedge(data_dir, sigma=0, use_mRNA=True, use_meth=True, use_miRNA=True, k_list=[10], is_prob=True, m_prob=1, edge_type='euclid'):
    ...
    def generate_G_from_H(H, variable_weight=False):
        ...
        W_e = np.mat(np.diag(W)) # shape: M X M (M = N * num_view)
        H = np.mat(H) # shape: N X M
        if variable_weight:
            return invDV2 * H, W_e, invDE * H.T * invDV2
        else:
            return invDV2 * H * W_e * invDE * H.T * invDV2 # shape: N X N
    ...
    H, WE, HT = generate_G_from_H(hyperedge_multi_omics, variable_weight=True)
    print('pre_calculate hypegraph G shape:', H.shape)
    print('label shape:', label.shape)
    return data_list, label, data_train_indices, data_test_indices, hyperedge_multi_omics, H, WE, HT
...
class HGNN_conv(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H: torch.Tensor, WE: torch.Tensor, HT: torch.Tensor):
        x = x.matmul(self.weight) # node feature transformation
        if self.bias is not None:
            x = x + self.bias
        G = H.matmul(WE).matmul(HT) # shape: N X N
...
class HGEMMD(nn.Module):
    ...
    def forward(self, data_list, H, WE, HT):
        ...
        MMfeature_hypergraph = self.HGNN(MMfeature_mmdynamics, H, WE, HT) # shape: N X hidden_dim[0]
    ...
    def forward_criterion(self, data_list, labeled_indices, unlabeled_indices, label, H, WE, HT, lambda_1=0.1, lambda_2=0.1):
        MMlogit, FeatureInfo, TCPLogit, TCPConfidence, ModalityEmbedding, MMfeature, MMfeature_hypergraph = self.forward(data_list, H, WE, HT)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_dir', default='./result/data_MOGONET_revise/HGEMMD_revise_dynamic/BRCA/', help='The output dir.')
    ...
    data_list, label, data_train_indices, data_test_indices, hyperedge_multi_omics, H, WE, HT = load_feature_and_hyperedge(args.data_dir, sigma=args.sigma, use_mRNA=use_mRNA, use_meth=use_meth, use_miRNA=use_miRNA, k_list=[k], is_prob=True, m_prob=1, edge_type='euclid')
    ...
    ###############################################################################################
    # Dynamics HGNN with trainable hyperedge weight
    ###############################################################################################
    WE = nn.Parameter(torch.FloatTensor(WE).to(device)) # trainable hyperedge weight, shape: M X M, M is the number of hyperedges
    HT = torch.FloatTensor(HT).to(device) # shape: N X N, N is the number of nodes
    ...
    for epoch in range(1, num_epoch + 1):
        model.train()
        optimizer.zero_grad()
        loss = model.forward_criterion(data_list=data_list, labeled_indices=data_train_indices, unlabeled_indices=data_test_indices, label=label, H=H, WE=WE, HT=HT, lambda_1=lambda_1, lambda_2=lambda_2)
        loss.backward()
        optimizer.step()
        scheduler.step()
        if epoch % 50 == 0:
            ...
            model.eval()
            with torch.no_grad():
                logit, _, _, _, _, _, _ = model.forward(data_list, H, WE, HT)
    ...
    model.eval()
    with torch.no_grad():
        logit = model.forward(data_list, H, WE, HT)[0]
        prob = F.softmax(logit, dim=1).data.cpu().numpy()
        label_test = label[data_test_indices]
        prob_test = prob[data_test_indices]
    if 'ROSMAP' in args.data_dir or 'LGG' in args.data_dir:
