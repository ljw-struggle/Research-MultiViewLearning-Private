...
def load_feature_and_graph(data_dir, sigma=0, use_mRNA=True, use_meth=True, use_miRNA=True):
    ...
    # Construct the multi-omics graph
    graph_mRNA = pd.DataFrame(data_list[0].T).corr(method='pearson').to_numpy() if use_mRNA else 0
    graph_meth = pd.DataFrame(data_list[1].T).corr(method='pearson').to_numpy() if use_meth else 0
    graph_miRNA = pd.DataFrame(data_list[2].T).corr(method='pearson').to_numpy() if use_miRNA else 0
    ...
    if sigma > 0:
        for i in range(len(data_list)):
            data_list[i] = data_list[i] + np.random.normal(0, args.sigma, data_list[i].shape)
...
class GEMMD(nn.Module):
    ...
    def forward(self, data_list, graph):
        ...
        MMfeature = torch.cat([i for i in ModalityEmbedding.values()], dim=1) # shape: N X (num_view*hidden_dim[0])
        MMfeature_graph = self.GNN(MMfeature, edge_index, edge_weight) # shape: N X hidden_dim[0]
        MMlogit = self.MMClasifier(torch.cat([MMfeature, MMfeature_graph], dim=1)) # shape: N X num_class
        return MMlogit
...
def count_gat_flops(module, input, output):
    ...
    # Each attention score needs: 2 * head_dim (concatenation) + 1 (linear transformation)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_dir', default='./result/data_MOGONET/GEMMD/BRCA/', help='The output dir.')
    ...
    parser.add_argument('-g', '--graph_model', default='GCN', type=str, help='The graph model to use. GCN or GAT.')
    ...
    data_list, label, data_train_indices, data_test_indices, graph = load_feature_and_graph(args.data_dir, sigma=args.sigma, use_mRNA=use_mRNA, use_meth=use_meth, use_miRNA=use_miRNA)
    ...
    model = GEMMD(dim_list, hidden_dim, num_class, dropout=0.5, graph_model=args.graph_model).to(device)
