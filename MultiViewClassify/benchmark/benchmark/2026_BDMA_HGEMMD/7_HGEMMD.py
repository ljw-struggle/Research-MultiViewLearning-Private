...
def load_feature_and_hyperedge(data_dir, sigma=0, use_mRNA=True, use_meth=True, use_miRNA=True, k_list=[10], is_prob=True, m_prob=1, edge_type='euclid'):
    ...
    # data_list = [data_list[i] for i in range(len(data_list)) if i == 0 and use_mRNA or i == 1 and use_meth or i == 2 and use_miRNA]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_dir', default='./result/data_MOGONET/HGEMMD/BRCA/', help='The output dir.')
    ...
    if 'TCGA-23' in args.data_dir:
        hidden_dim = [500]; num_epoch = 2500; lr = 1e-4; step_size = 500; num_class = 23; lambda_1 = args.lambda_1; lambda_2 = args.lambda_2; k = args.k
    ...
    label = torch.LongTensor(label).to(device) # shape: (num_cells, )
    pre_calc_G = torch.FloatTensor(pre_calc_G).to(device) # shape: (num_cells, num_cells)
    ...
    if args.verbose == 2:
        ...
        flops, params = profile(model, inputs=(data_list, pre_calc_G), verbose=True, custom_ops={HGNN_conv: count_gcn_flops}) # return MACs and params; flops = 2 * MACs
        ...
        print(f'Input graph shape: {pre_calc_G.shape}')
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        if epoch % 50 == 0:
            ...
            if 'TCGA-23' in args.data_dir:
                ...
                print('Training Epoch {:d}: Train ACC={:.5f}, F1_weighted={:.5f}, F1_macro={:.5f}, AP={:.5f}, AUC={:.5f}'.format(epoch, acc, f1_weighted, f1_macro, ap, auc))
            with torch.no_grad():
                logit, _, _, _, _, _, _ = model.forward(data_list, pre_calc_G)
                prob = F.softmax(logit, dim=1).data.cpu().numpy()
            ...
            if 'TCGA-23' in args.data_dir:
                ...
                ap = average_precision_score(np.eye(num_class)[label_test.cpu().numpy()], prob_test, average='macro')
                auc = roc_auc_score(np.eye(num_class)[label_test.cpu().numpy()], prob_test, average='macro')
                print('Training Epoch {:d}: Test ACC={:.5f}, F1_weighted={:.5f}, F1_macro={:.5f}, AP={:.5f}, AUC={:.5f}'.format(epoch, acc, f1_weighted, f1_macro, ap, auc))
    ...
    if 'TCGA-23' in args.data_dir:
        ...
        if not os.path.exists(os.path.join(args.output_dir, 'metrics.csv')):
            metrics_df = pd.DataFrame({'seed': [args.seed], 'k': [args.k], 'l1': [args.lambda_1], 'l2': [args.lambda_2], 'modality': [args.modality], 'sigma': [args.sigma], 'ACC': [acc], 'F1_weighted': [f1_weighted], 'F1_macro': [f1_macro], 'macro_AUPR': [ap], 'macro_AUC': [auc]})
        else:
            metrics_df = pd.read_csv(os.path.join(args.output_dir, 'metrics.csv'))
            metrics_df = pd.concat([metrics_df, pd.DataFrame({'seed': [args.seed], 'k': [args.k], 'l1': [args.lambda_1], 'l2': [args.lambda_2], 'modality': [args.modality], 'sigma': [args.sigma], 'ACC': [acc], 'F1_weighted': [f1_weighted], 'F1_macro': [f1_macro], 'macro_AUPR': [ap], 'macro_AUC': [auc]})], ignore_index=True)
        metrics_df.to_csv(os.path.join(args.output_dir, 'metrics.csv'), index=False)
        
        if args.verbose == 1:
            if len(metrics_df) >= 10:
                mean_values = metrics_df[['ACC', 'F1_weighted', 'F1_macro', 'macro_AUPR', 'macro_AUC']].mean()
                variance_values = metrics_df[['ACC', 'F1_weighted', 'F1_macro', 'macro_AUPR', 'macro_AUC']].var() # ddof=1: sample variance
                # std_values = metrics_df[['ACC', 'F1_weighted', 'F1_macro', 'macro_AUPR', 'macro_AUC']].std() # ddof=1: sample std
