...
class SiameseNet(nn.Module):
    def __init__(self, input_dim, arch, dropout=0.0):
        super().__init__()
        act = {"relu": lambda: nn.ReLU(), "tanh": lambda: nn.Tanh(), "softmax": lambda: nn.Softmax(dim=-1), "softplus": lambda: nn.Softplus(), "selu": lambda: nn.SELU()}
        layers = []
        in_dim = input_dim
...
class MvSCN(nn.Module):
    def __init__(self, input_dims, arch, n_clusters, view_size, dropout=0.0):
        ...
        for i in range(view_size):
            ...
            self.towers.append(seq); self.orthogonals.append(ortho)
...
if __name__ == "__main__":
    ...
    def train_siamese(model, pairs_train, dist_train, lr=0.0001, drop=0.1, patience=15, num_epochs=400, batch_size=128, device=None):
        ...
        best_loss = float("inf")
        wait = 0
        x1 = torch.from_numpy(pairs_train[:, 0]).float(); x2 = torch.from_numpy(pairs_train[:, 1]).float(); y = torch.from_numpy(dist_train).float().unsqueeze(1)
        ...
        for epoch in range(num_epochs):
            ...
            for x1_b, x2_b, y_b in loader:
                x1_b = x1_b.to(device); x2_b = x2_b.to(device); y_b = y_b.squeeze(1).to(device)
                optimizer.zero_grad()
            ...
            if avg_loss <= best_loss:
                ...
                if wait > patience:
                    ...
                    if g["lr"] <= 1e-8:
                        print("STOPPING EARLY (lr too small)"); break
            print("Epoch: {}, loss={:.4f}".format(epoch, avg_loss))
    for i in range(view_size):
        x_train_i, _, x_test_i, _ = data_list[i]["spectral"]
        x_train_list.append(x_train_i); x_test_list.append(x_test_i)
        dim = int(np.prod(x_train_i.shape[1:])); input_dims.append(dim)
    ...
    def train_mvscn(model, siamese_list, x_train_list, n_nbrs, scale_nbr, batch_size, lamb, lr=0.0001, drop=0.1, patience=15, num_epochs=400, batches_per_epoch=100, device=None):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = model.to(device)
        siamese_list = [s.to(device) for s in siamese_list]
        optimizer = torch.optim.RMSprop(model.parameters(), lr=lr)
        scheduler_stage = 0
        best_loss = float("inf")
        wait = 0
