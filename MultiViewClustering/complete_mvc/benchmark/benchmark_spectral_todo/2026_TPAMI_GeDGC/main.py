...
class vae(nn.Module):
    def __init__(self, layer_sizes, activation=nn.ReLU()):
        ...
        self.encoder = nn.Sequential(*encoder); self.decoder = nn.Sequential(*decoder)
    ...
    def get_latent(self, inputs):
        ...
        mean = self.mean(h); logvar = self.logvar(h)
        return mean, logvar
...
def train_model(vae, classifier, GMM, optimizer, lr_scheduler, dataloader, dataname, epoch_num, device):
    vae = vae.to(device); classifier = classifier.to(device); GMM = GMM.to(device)
    avg_loss = []
    early_stopping = EarlyStopping(patience=10, delta=1e-3)
    t = trange(epoch_num, leave=True)
    for epoch in t:
        Total_loss = []; label = []; label_pred = []; Recon_loss = []
        for batch_idx, (inputs, labels) in enumerate(dataloader):
            ...
            for data_v in inputs:
                ...
                temp_data1 = torch.cat(temp_data1, dim=0); temp_data2 = torch.cat(temp_data2, dim=0)
                all_inputs.append(temp_data1); all_targets.append(temp_data2)
            label.append(labels)
            ...
            # Begin training
            vae.train(); classifier.train(); GMM.train()
            loss = 0
            J = 1
        ...
        t.set_description('|Epoch:{} Total loss={:3f} Reconstruction Loss={:6f} PUR={:5f} ARI={:5f} NMI={:5f} ACC={:6f}'.format(epoch + 1, np.mean(Total_loss), np.mean(Recon_loss), pur, ARI, NMI, acc))
        t.refresh()
...
def train_opt(data, label, dataname):
    batch_size = 128 if data[0].shape[0] > 1024 else 16
    learning_rate = 0.0001; weight_decay = 1e-6; step_size = 50; gama = 0.1; epoch = 1000
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Load pretrained model")
    vae = torch.load('Intermediate_data/' + dataname + '/pretrained_vae.pkl'); classifier = torch.load('Intermediate_data/' + dataname + '/pretrained_classifier.pkl'); GMM = torch.load('Intermediate_data/' + dataname + '/pretrained_GMM.pkl')
    print("Train the model")
    dataloader = DataLoader(Cell(data, label), batch_size=batch_size, shuffle=True)
    optimizer = torch.optim.Adam(list(vae.parameters()) + list(classifier.parameters()) + list(GMM.parameters()), lr=learning_rate, weight_decay=weight_decay)
    ...
    vae.eval(); classifier.eval(); GMM.eval()
    ...
    vae = vae.to('cpu'); classifier = classifier.to('cpu'); GMM = GMM.to('cpu')
    torch.save(vae, 'result/' + dataname + '/trained_vae.pkl'); torch.save(classifier, 'result/' + dataname + '/trained_classifier.pkl'); torch.save(GMM, 'result/' + dataname + '/trained_GMM.pkl')
...
def pretrain_opt(data, label, c, dataname, train_vae = True):
    batch_size = 128 if data[0].shape[0] > 1024 else 16
    learning_rate = 0.001; weight_decay = 1e-6; step_size = 50; gama = 0.1; epoch = 100
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if train_vae:
        activation = nn.ReLU()
        encoder_sizes = []
        ...
        GMM = GMM_Model(c, c, mean.t(), var.t()).to(device)
        GMM.eval()
        ...
        vae = vae.to('cpu'); GMM = GMM.to('cpu')
        torch.save(vae, 'Intermediate_data/' + dataname + '/pretrained_vae.pkl'); torch.save(GMM, 'Intermediate_data/' + dataname + '/pretrained_GMM.pkl')
    else:
        vae = torch.load('Intermediate_data/' + dataname + '/pretrained_vae.pkl'); GMM = torch.load('Intermediate_data/' + dataname + '/pretrained_GMM.pkl')
    vae = vae.to(device); GMM = GMM.to(device); vae.eval(); GMM.eval()
    dataloader = DataLoader(Cell(data), batch_size=1024, shuffle=False)
    x_mean = []
    for batch_idx, (inputs) in enumerate(dataloader):
        inputs = [input_.to(device) for input_ in inputs]
    ...
    for batch_idx, (inputs) in enumerate(dataloader):
        ...
        with torch.no_grad():
            x_m, x_v = vae.get_latent(inputs)
        x_mean.append(x_m); x_logvar.append(x_v)
    x_mean = torch.cat(x_mean, dim=0); x_logvar = torch.cat(x_logvar, dim=0)
    x_sample = gen_x(x_mean, torch.exp(0.5 * x_logvar))
    with torch.no_grad():
        pred_label = torch.max(classifier(x_sample), dim=-1)
    acc, _ = cluster_acc(to_numpy(pred_label[-1]), label)
    ...
    vae = vae.to('cpu'); classifier = classifier.to('cpu')
...
if __name__ == '__main__':
    ...
    dataname = 'Chen_high'
    os.makedirs('Intermediate_data/Chen_high', exist_ok=True)
    os.makedirs('result/Chen_high', exist_ok=True)
    for t in range(1):
        setup_seed(t)
        ...
        neighbors = cal_neighbors_D(spectralNet_dist, K=3)
        data = form_data(raw_data, label, neighbors)
        acc, NMI, ARI, pur, FMI = train_opt(data, label, dataname)
        print('| ACC = {:6f} NMI = {:6f} ARI = {:6f} Purity = {:6f} FMI = {:6f}'.format(acc, NMI, ARI, pur, FMI))
