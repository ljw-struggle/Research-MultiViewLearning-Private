...
def benchmark_2011_JMLR_SKLEARN_KMeans(dataset_name='BDGP', 
                                       init='k-means++', 
                                       n_init=10, 
                                       seed=42,
                                       verbose=False):
    ...
    if data_size < 10000:
        model = KMeans(n_clusters=class_num, init=init, n_init=n_init, random_state=seed)
    else:
        model = MiniBatchKMeans(n_clusters=class_num, init=init, n_init=n_init, batch_size=1024, random_state=seed)
    y_pred = model.fit_predict(data)
    nmi, ari, acc, pur = evaluate(label, y_pred)
    print("Clustering on concatenated views (KMeans):") if verbose else None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init", default="k-means++", type=str, help="Initialization method")
    parser.add_argument("--n_init", default=10, type=int, help="Number of initializations")
    parser.add_argument("--seed", default=42, type=int)
    args = parser.parse_args()
    nmi, ari, acc, pur = benchmark_2011_JMLR_SKLEARN_KMeans(
