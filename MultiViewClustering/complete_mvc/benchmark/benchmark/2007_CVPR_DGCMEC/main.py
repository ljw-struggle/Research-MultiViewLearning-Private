...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='TEA', type=str)
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dataset_name = args.dataset; batch_size = args.batch_size; temperature_f = args.temperature_f; temperature_l = args.temperature_l; learning_rate = args.learning_rate; weight_decay = args.weight_decay; workers = args.workers; 
    ...
    for epoch in range(tune_epochs):
        ...
        loss.backward()
        optimizer.step()
        print('Pseudo-label Refinement Epoch {}'.format(epoch), 'Loss:{:.6f}'.format(loss.item()/19))
    validation(model, dataset, view, data_size, class_num, eval_multi_view=False)
