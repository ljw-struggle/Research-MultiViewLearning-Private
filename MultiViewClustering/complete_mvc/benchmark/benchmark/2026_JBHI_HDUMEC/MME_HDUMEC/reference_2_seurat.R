...
for (dataset in c("BRCA", "KIPAN", "LGG", "DOGMA", "TEA", "NEAT")) {
    if (dataset %in% c("BRCA", "KIPAN", "LGG")) {
        ...
        multiome <- FindMultiModalNeighbors(obj, reduction.list = list("rpca", "apca", "lsi"), dims.list = list(1:10, 1:10, 2:10))
        # Run UMAP on WNN graph to generate embedding
        multiome <- RunUMAP(multiome, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_")
        # Extract embedding: rows = samples, columns = embedding dimensions
        embedding_seurat <- Seurat::Embeddings(multiome, reduction="wnn.umap")
        # Convert to data.frame and add label as first column
        embedding_df <- as.data.frame(embedding_seurat)
        # Ensure label order matches embedding row order (by sample index)
        embedding_df$label <- label[as.integer(gsub("sample-", "", rownames(embedding_seurat)))]
        # Reorder columns: label first, then embedding dimensions
        embedding_df <- embedding_df[, c("label", setdiff(colnames(embedding_df), "label"))]
        # Save embedding: first column = label, other columns = embedding dimensions
        write.table(embedding_df, row.names = FALSE, col.names = TRUE, paste0(data_dir, dataset, "/embedding_seurat.csv"), quote = FALSE, sep = ",")
        ...
        multiome <- FindMultiModalNeighbors(obj, reduction.list = list("rpca", "apca", "lsi"), dims.list = list(1:rpca_dims, 1:apca_dims, lsi_dims_list))
        # Run UMAP on WNN graph to generate embedding
        multiome <- RunUMAP(multiome, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_")
        # Extract embedding: rows = samples, columns = embedding dimensions
        embedding_seurat <- Seurat::Embeddings(multiome, reduction="wnn.umap")
