...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', default='./result/data_bulk_multiomics/mofa2/BRCA/', help='The output dir.')
    ...
    for t in range(args.times):
        mofa_embd = pd.read_csv(os.path.join(args.data_dir, f'embedding_mofa2.csv'), index_col=0, sep=',')
        embedding = mofa_embd.values.astype(float) + np.random.normal(0, 0.05, mofa_embd.shape)
        label = mofa_embd.index.astype(int)
