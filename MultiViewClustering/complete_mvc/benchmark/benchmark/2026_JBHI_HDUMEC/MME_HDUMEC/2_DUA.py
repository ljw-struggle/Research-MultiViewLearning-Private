...
class DUA(nn.Module):
    def __init__(self, embed_dim=200, num_samples=10000, num_views=3, feature_dims=[1000, 1000, 500], hidden_dims=[512, 512, 512]):
        ...
        # self.register_parameter('H', self.H) # register the latent space H as a parameter of the model if don't use self.H.
        ...
        self.uncertainty_net_list = nn.ModuleList([nn.Sequential(nn.Linear(embed_dim, hidden_dims[i]), nn.BatchNorm1d(hidden_dims[i]), nn.ReLU(), 
                                                                 nn.Linear(hidden_dims[i], 1)) for i in range(num_views)]) # predict uncertainty for each view
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', default='./result/data_bulk_multiomics/DUA/BRCA/', help='The output dir.')
    ...
    for t in range(args.times):
        ...
        model = DUA(embed_dim=args.latent_dim, num_samples=data_samples, num_views=data_views, feature_dims=data_features, hidden_dims=[512, 512, 512]).to(device)
