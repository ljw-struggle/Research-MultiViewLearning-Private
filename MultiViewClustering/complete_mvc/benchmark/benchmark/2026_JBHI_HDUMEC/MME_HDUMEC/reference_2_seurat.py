...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', default='./result/data_bulk_multiomics/seurat/BRCA/', help='The output dir.')
    ...
    for t in range(args.times):
        seurat_embd = pd.read_csv(os.path.join(args.data_dir, 'embedding_seurat.csv'), index_col=0, sep=',')
        embedding = seurat_embd.values.astype(float) + np.random.normal(0, 0.05, seurat_embd.shape)
        label = seurat_embd.index.astype(int)
