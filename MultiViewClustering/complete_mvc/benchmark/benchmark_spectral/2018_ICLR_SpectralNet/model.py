...
########################################################
# Models and Losses: Autoencoder, Siamese, Spectral, SpectralNetLoss, ContrastiveLoss
########################################################
...
class SiameseNetModel(nn.Module):
    def __init__(self, architecture: dict, input_dim: int):
        ...
        for layer in self.architecture:
            self.layers.append(nn.Sequential(nn.Linear(current_dim, layer), nn.ReLU()))
            current_dim = layer
...
class SpectralNetModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, should_update_orth_weights: bool = True) -> torch.Tensor:
        # Perform the forward pass of the model. This function takes an input tensor `x` and computes the forward pass of the model.
        # If `should_update_orth_weights` is set to True, the orthonormalization weights are updated using the QR decomposition. The output tensor is returned.
        for layer in self.layers:
            x = layer(x)
        Y_tilde = x
...
class AETrainer:
    ...
    def embed(self, X: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            encoded_data = self.ae_net.encode(X)
        return encoded_data
...
class SiameseTrainer:
    ...
    def train(self, X: torch.Tensor) -> SiameseNetModel:
        ...
        for epoch in t:
            ...
            for x1, x2, label in train_loader:
                x1 = x1.to(self.device).view(x1.size(0), -1)
                x2 = x2.to(self.device).view(x2.size(0), -1)
    ...
    def _get_pairs(self) -> list: # Gets the pairs of data points to be used for training the siamese network.
...
class SpectralTrainer:
    ...
    def train(self, X: torch.Tensor, y: torch.Tensor, siamese_net: nn.Module = None) -> SpectralNetModel:
        ...
        for epoch in t:
            ...
            for (X_grad, _), (X_orth, _) in zip(train_loader, ortho_loader):
                ...
                if self.is_sparse:
                    X_grad = self.make_batch_for_sparse_grapsh(X_grad)
                    X_orth = self.make_batch_for_sparse_grapsh(X_orth)
    ...
    def validate(self, valid_loader: DataLoader) -> float:
        ...
        with torch.no_grad():
            for batch in valid_loader:
                ...
                if self.is_sparse:
                    X = self.make_batch_for_sparse_grapsh(X)
    ...
    def _get_affinity_matrix(self, X: torch.Tensor) -> torch.Tensor:
        ...
        Dx = torch.cdist(X, X)
        nbrs = NearestNeighbors(n_neighbors=n_neighbors + 1).fit(X.cpu().detach().numpy())
        Dis, Ids = nbrs.kneighbors(X.cpu().detach().numpy()) # return the distances and the indices of the k nearest neighbors of each data point
        scale = self.compute_scale(Dis, k=scale_k, is_local=is_local)
        W = self.get_gaussian_kernel(Dx, scale, Ids, device=self.device, is_local=is_local)
        return W
    ...
    def get_gaussian_kernel(self, D: torch.Tensor, scale, Ids: np.ndarray, device: torch.device, is_local: bool = True) -> torch.Tensor:
    ...
    def compute_scale(self,Dis: np.ndarray, k: int = 2, med: bool = True, is_local: bool = True) -> np.ndarray:
        ...
        if is_local:
            scale = np.max(Dis, axis=1) if not med else np.median(Dis, axis=1)
        else:
            scale = np.max(Dis[:, k - 1]) if not med else np.median(Dis[:, k - 1])
        return scale
