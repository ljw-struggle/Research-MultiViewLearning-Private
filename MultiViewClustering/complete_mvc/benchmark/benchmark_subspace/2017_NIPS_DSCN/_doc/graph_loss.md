...
最小化此损失的效果：**让编码器学出的特征表示保持自表示系数矩阵所描述的邻域结构**。虽然 L 的非对角元素是 $-S_{ij}$（负的），但经过二次型展开后，减号和 D 的正项合并，最终恰好变成 $S_{ij}$ 乘以距离平方——这就是拉普拉斯矩阵的精妙之处。直觉上 L 看起来是在"减去"相似性，但二次型的整体效果是在用相似性作为正权重来惩罚距离。
