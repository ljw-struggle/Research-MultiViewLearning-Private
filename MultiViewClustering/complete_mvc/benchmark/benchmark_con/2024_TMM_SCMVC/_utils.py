...
def load_data(dataset):
    match dataset:
        ...
        case "Prokaryotic":
            dataset = prokaryotic('./data/prokaryotic.mat'); dims = [438, 3, 393]; view = 3; data_size = 551; class_num = 4
        case "Cifar10":
            dataset = cifar_10('./data/cifar10.mat'); dims = [512, 2048, 1024]; view = 3; data_size = 50000; class_num = 10
        case "Cifar100":
            dataset = cifar_100('./data/cifar100.mat'); dims = [512, 2048, 1024]; view = 3; data_size = 50000; class_num = 100
        case _:
...
class cifar_10():
    def __init__(self, path):
        ...
        self.y = data['truelabel'][0][0].astype(np.int64).reshape(50000,) # shape: (50000,)
...
class synthetic3d():
    def __init__(self, path):
        data = sio.loadmat(path)
        self.x_1 = data['X'][0][0].astype(np.float32) # shape: (600, 3)
        self.x_2 = data['X'][1][0].astype(np.float32) # shape: (600, 3)
        self.x_3 = data['X'][2][0].astype(np.float32) # shape: (600, 3)
...
class prokaryotic():
    def __init__(self, path):
        data = sio.loadmat(path)
        self.x_1 = data['X'][0][0].astype(np.float32) # shape: (551, 438)
        self.x_2 = data['X'][1][0].astype(np.float32) # shape: (551, 3)
        self.x_3 = data['X'][2][0].astype(np.float32) # shape: (551, 393)
        self.y = data['Y'].astype(np.int64).reshape(551,) # shape: (551,)
