...
def benchmark_2023_NIPS_SEM(dataset_name="Caltech",
                            feature_dim=512,
                            ...
                            weight_decay=0.0,
                            temperature_f=1.0,
                            reconstruction_training_method='AE',
                            contrastive_training_method='InfoNCE',
                            mse_epochs=100,
                            con_epochs=100,
                            lambda_reconstruction=1,
                            measurement='CMI',
                            weight_update_iteration=4,
                            seed=5,
                            verbose=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    acc, nmi, ari, pur, _, _ = validation(model, dataset, view, data_size, class_num, eval_z=False, eval_h=True, measure_type=measurement)
...
if __name__ == "__main__":
    ...
    if args.dataset == "DHA": args.con_epochs = 300; args.weight_update_iteration = 1
    if args.dataset == "CCV": args.con_epochs = 50; args.weight_update_iteration = 4
    if args.dataset == "YoutubeVideo": args.con_epochs = 25; args.weight_update_iteration = 1
    if args.dataset == "NUSWIDE": args.con_epochs = 25; args.weight_update_iteration = 4
    if args.dataset == "Caltech": args.con_epochs = 100; args.weight_update_iteration = 3
    
    nmi, ari, acc, pur = benchmark_2023_NIPS_SEM(
        ...
        reconstruction_training_method=args.reconstruction_training_method,
        contrastive_training_method=args.contrastive_training_method,
        mse_epochs=args.mse_epochs,
        con_epochs=args.con_epochs,
        lambda_reconstruction=args.lambda_reconstruction,
        measurement=args.measurement,
        weight_update_iteration=args.weight_update_iteration,
        seed=args.seed,
        verbose=False
    )
