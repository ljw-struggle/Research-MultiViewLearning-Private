...
class Trainer:
    def __init__(self, args):
        ...
        self.model = GATE(args.hidden_dims_1, args.hidden_dims_2, args.lambda_, num_nodes=num_nodes).to(self.device)
    ...
    def __call__(self, A, A2, X, X2, S, S2, R, R2, L):
        ...
        while pre_epoch < 1:
            ...
            pre_loss, _, structure_loss, features_loss, _, consistent_loss, cRegular, SE_loss, _, _, _, _ = self.model(A, A2, X, X2, S, S2, R, R2, p_dummy, Theta, None)
            pre_loss.backward()
            ...
            if pre_epoch % 5 == 0:
                print("-------------------------------------------------------------")
                print("pre_epoch: %d" % pre_epoch, "Pre_Loss: %.2f" % pre_loss.item(), "ReLoss-X: %.2f" % features_loss.item(), "ReLoss-A: %.2f" % structure_loss.item(), "pre_SE_loss: %.2f" % SE_loss.item(), "Pre_consistent_loss: %.2f" % consistent_loss.item(), "cRegular: %.2f" % cRegular.item())
        with torch.no_grad():
        ...
        print("acc: {:.8f}\t\tnmi: {:.8f}\t\tf_score: {:.8f}\t\tari: {:.8f}".format(acc_x, nmi(L, y_x + 1), f_score(L, y_x + 1), ari(L, y_x + 1)))
        epoch = 0
        ...
        while epoch < self.args.n_epochs:
            ...
            pre_loss, cost, dense_loss, features_loss, structure_loss, SE_loss, s2_Coef, consistent_loss, cRegular, cqLoss, H, H2 = self.model(A, A2, X, X2, S, S2, R, R2, p_pt, Theta_pt, Y)
            cost.backward()
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.gradient_clipping)
            self.optimizer.step()
            cost = cost.item()
            ...
            s2_nmi_x = nmi(L, Y + 1)
            s2_ari_x = ari(L, Y + 1)
            print("epoch: %d" % epoch, "Total_Loss: %.2f" % cost, "ReLoss-X: %.2f" % f_loss, "ReLoss-A: %.2f" % st_loss, "SeLoss: %.2f" % SELoss, "CrossELoss: %.2f" % CrossELoss, "consistent_loss: %.2f" % consistent_loss, "cRegular: %.2f" % cRegular, "cqLoss: %.2f" % cqLoss)
            ...
            fh2.write('Fin_epoch=%d, Fin_Total: %.2f, Re_Loss: %.2f, SEloss: %.2f, dense_loss: %.2f, consistent_loss: %.2f, cRegular: %.2f, Cq_loss: %.2f ' % (epoch, cost, f_loss + st_loss, SELoss, CrossELoss, consistent_loss, cRegular, cqLoss))
            fh2.write('\r\n')
            fh2.flush()
            fh2.close()
