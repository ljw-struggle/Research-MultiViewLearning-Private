...
def benchmark_2013_ICML_DCCA(dataset_name="BDGP", 
                             outdim_size=10, 
                             regularization=1e-3, 
                             batch_size=256, 
                             learning_rate=1e-3, 
    ...
    assert view > 1, "DCCA(DGCCA) only supports two views or more, but got {view} views"
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=True)
    dim_list_multi_view = [[dims[v], 128, 64, 32, outdim_size] for v in range(view)]
    model = DeepCCA(dim_list_multi_view).to(device)
    if view == 2:
    ...
    for epoch in range(num_epochs):
        ...
        for x_list, y, idx in dataloader:
            ...
            optimizer.zero_grad()
            z_list = model(x_list)
            loss = criterion(z_list[0], z_list[1]) if view == 2 else criterion(z_list)
            loss.backward()
            optimizer.step()
            losses.append(loss.item())
        print('Training Epoch {}'.format(epoch), 'Loss:{:.6f}'.format(np.mean(losses))) if verbose else None
...
if __name__ == "__main__":
    ...
    nmi, ari, acc, pur = benchmark_2013_ICML_DCCA(
