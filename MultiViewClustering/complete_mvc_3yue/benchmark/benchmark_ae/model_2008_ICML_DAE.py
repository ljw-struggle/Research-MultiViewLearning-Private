...
class DAE(nn.Module):
    def __init__(self, input_dim, latent_dim=10, noise_type='mask', masking_rate=0.1, gaussian_std=0.1):
    ...
    def add_noise(self, input):
        if self.noise_type == 'mask': # Masking noise: randomly set features to 0
            if self.masking_rate == 0.0:
                return input
            mask = torch.rand_like(input) > self.masking_rate
            return input * mask
        if self.noise_type == 'gaussian': # Gaussian noise: add Gaussian noise
            if self.gaussian_std == 0.0:
                return input
            noise = torch.randn_like(input) * self.gaussian_std
            return input + noise
        return input
    ...
    def forward(self, input):
        if self.training:
            ...
            latent = self.encode(input_noisy) # shape: (batch_size, latent_dim) -> latent
...
def benchmark_2008_ICML_DAE(dataset_name="BDGP", 
                            latent_dim=10, 
                            noise_type="gaussian", 
                            masking_rate=0.1, 
                            gaussian_std=0.1, 
    ...
    model = DAE(input_dim=input_dim, latent_dim=latent_dim, noise_type=noise_type, masking_rate=masking_rate, gaussian_std=gaussian_std).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (x_list, y, idx) in enumerate(dataloader):
            ...
            recon, latent = model(x_cat) # shape: (batch_size, input_dim), (batch_size, latent_dim) -> recon, latent
            loss = F.mse_loss(recon, x_cat, reduction='sum') # shape: scalar, reconstruction loss (reconstruct clean input from noisy input)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noise_type", default="gaussian", type=str, choices=["mask", "gaussian"], help="Noise type: 'mask' (masking noise), 'gaussian' (Gaussian noise)")
    parser.add_argument("--masking_rate", default=0.1, type=float, help="Masking rate for masking noise")
    parser.add_argument("--gaussian_std", default=0.1, type=float, help="Standard deviation for gaussian noise")
    parser.add_argument("--batch_size", default=256, type=int)
    parser.add_argument("--epochs", default=100, type=int)
    parser.add_argument("--lr", default=5e-4, type=float)
    parser.add_argument("--seed", default=2, type=int)
    args = parser.parse_args()
    nmi, ari, acc, pur = benchmark_2008_ICML_DAE(
