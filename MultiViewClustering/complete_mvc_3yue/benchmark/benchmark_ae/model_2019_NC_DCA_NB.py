...
class DCA(nn.Module):
    def __init__(self, input_dim, latent_dim):
        ...
        self.decoder = NBDecoder(latent_dim, input_dim)

    def forward(self, x):
        latent = self.encoder(x)
        mu, theta = self.decoder(latent)
        return mu, theta, latent
...
class NBLoss(nn.Module):
    ...
    def forward(self, x, mu, theta):
        """ Negative Log-Likelihood Loss for Negative Binomial Distribution (NB)
        ...
        ## NB Loss by manual implementation:
        log_negbinom = torch.lgamma(theta + x) - torch.lgamma(theta) - torch.lgamma(x + 1) + \
                       theta * (torch.log(theta) - torch.log(theta + mu)) + x * (torch.log(mu) - torch.log(theta + mu)) # shape: [batch_size, feature_dim]
        loss = -log_negbinom.mean() # shape: [1]
        return loss
        
        ## NB Loss by torch.distributions.NegativeBinomial API implementation: (Same as manual implementation)
        # total_count = theta
        # logits = torch.log(mu) - torch.log(theta)
        # dist = NegativeBinomial(total_count=total_count, logits=logits, validate_args=False)
        # loss = -dist.log_prob(x).mean() # shape: [batch_size, feature_dim] -> shape: [1]
        # return loss
...
def benchmark_2019_NC_DCA_NB(dataset_name="BDGP",
                               latent_dim=20,
                               batch_size=256,
                               num_epochs=100,
                               learning_rate=5e-4,
    ...
    ## Attention: For DCA-NB model: the input x is the count data (non-negative integers).
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (x_list, y, idx) in enumerate(dataloader):
            x_cat = torch.cat([x.to(device) for x in x_list], dim=1)
            optimizer.zero_grad()
            mu, theta, z = model(x_cat)
            loss = criterion(x_cat, mu, theta)
            loss.backward()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DCA-NB")
    ...
    nmi, ari, acc, pur = benchmark_2019_NC_DCA_NB(
