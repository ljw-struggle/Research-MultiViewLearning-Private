...
def scale_normalize_matrix(input_matrix, min_value=0, max_value=1):
    if torch.is_tensor(input_matrix):
        input_matrix_np = input_matrix.detach().cpu().numpy()
        min_val = input_matrix_np.min(); max_val = input_matrix_np.max()
        scaled_matrix = (input_matrix_np - min_val) / (max_val - min_val) * (max_value - min_value) + min_value
        return torch.from_numpy(scaled_matrix).to(input_matrix.device)
    else:
...
def validation(model, dataset, view, data_size, class_num, eval_q=False, eval_h=False, verbose=False):
    ...
    # Return result based on eval_q and eval_h
    if eval_q == True:
        q_pred = np.argmax(np.array(q), axis=1)
        nmi, ari, acc, pur = evaluate(y, q_pred)
        print('Evaluation on cluster assignments: ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}'.format(acc, nmi, ari, pur)) if verbose else None
    if eval_h == True:
        h_pred = KMeans(n_clusters=class_num, n_init='auto').fit_predict(h)
        nmi, ari, acc, pur = evaluate(y, h_pred)
        print('Evaluation on feature vectors: ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}'.format(acc, nmi, ari, pur)) if verbose else None
...
def destiny_peak(model, dataset, view, data_size, device, gamma, alpha, beta, metric='euclidean'): # density peak clustering !!!!!
...
def benchmark_2024_IJCAI_SCM(dataset_name="MNIST-USPS",
                             batch_size=256,
                             temperature_f=0.5,
                             learning_rate=0.0003,
                             weight_decay=0.0,
                             stage_1_iterations=50,
                             stage_2_iterations=200,
                             feature_dim=256,
    ...
    for epoch in range(stage_1_iterations):
        ...
        loss.backward()
        optimizer.step()
        print("Epoch: {} Total Loss: {:.4f}".format(epoch + 1, loss.item())) if verbose else None
    
    ## 3.2. Stage 2: Fine-tuning with pseudo-labels (if mode includes ETC)
    if mode == 'SCM_ETC' or mode =='SCM_REC_ETC':
        probability_matrix = destiny_peak(model, dataset, view, data_size, device, gamma, alpha, beta) # shape: (data_size, 64), probability_matrix is the probabilities of the data
    ...
    # If mode includes ETC, use eval_q (cluster assignments), otherwise use eval_h (feature vectors)
    if mode == 'SCM_ETC' or mode == 'SCM_REC_ETC':
        nmi, ari, acc, pur = validation(model, dataset, view, data_size, class_num, eval_q=True, eval_h=False, verbose=verbose)
    else:
        nmi, ari, acc, pur = validation(model, dataset, view, data_size, class_num, eval_q=False, eval_h=True, verbose=verbose)
    return nmi, ari, acc, pur
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mode", default="SCM_REC_ETC", type=str, choices=["SCM", "SCM_REC", "SCM_REC_ETC", "SCM_ETC"])
    parser.add_argument("--seed", default=10, type=int)
    args = parser.parse_args()
    
    ## Adjust parameters based on dataset
    if args.dataset == "MNIST-USPS": args.stage_1_iterations = 1000; args.stage_2_iterations = 1000; args.gamma = 0.02; args.alpha = 0.5; args.beta = 0.5; args.seed = 1
    if args.dataset == "BDGP": args.stage_1_iterations = 400; args.stage_2_iterations = 3000; args.gamma = 0.1; args.alpha = 0.5; args.beta = 0.5; args.seed = 4
    if args.dataset == "Fashion": args.stage_1_iterations = 20000; args.stage_2_iterations = 2500; args.gamma = 0.003; args.alpha = 0.2; args.beta = 0.81; args.seed = 1
    if args.dataset == "DHA": args.stage_1_iterations = 500; args.stage_2_iterations = 700; args.gamma = 0.02; args.alpha = 0.2; args.beta = 0.5; args.seed = 4
    if args.dataset == "WebKB": args.stage_1_iterations = 200; args.stage_2_iterations = 200; args.gamma = 0.001; args.alpha = 0.6; args.beta = 0.6; args.seed = 2
    if args.dataset == "NGs": args.stage_1_iterations = 200; args.stage_2_iterations = 800; args.gamma = 0.00005; args.alpha = 0.5; args.beta = 0.5; args.seed = 5
    if args.dataset == "VOC": args.stage_1_iterations = 200; args.stage_2_iterations = 900; args.gamma = 0.002; args.alpha = 0.01; args.beta = 0.37; args.seed = 9
    if args.dataset == "Fc_COIL_20": args.stage_1_iterations = 2000; args.stage_2_iterations = 400; args.gamma = 0.031; args.alpha = 0.2; args.beta = 0.5; args.seed = 1
    
    nmi, ari, acc, pur = benchmark_2024_IJCAI_SCM(
        dataset_name=args.dataset,
        batch_size=args.batch_size,
        temperature_f=args.temperature_f,
        learning_rate=args.learning_rate,
        weight_decay=args.weight_decay,
        stage_1_iterations=args.stage_1_iterations,
        stage_2_iterations=args.stage_2_iterations,
