import os, sys, math, faiss, random, argparse, scipy.sparse as sp, numpy as np # faiss: (Facebook AI Similarity Search) is a library for efficient similarity search and clustering of dense vectors.
import torch
...
def validation(model, dataset, view, data_size, class_num, verbose=False):
    ...
    print("Clustering on latent q_fusion (k-means):") if verbose else None
    print("ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}".format(acc_q, nmi_q, ari_q, pur_q)) if verbose else None
    print("Clustering on latent h_fusion (k-means):") if verbose else None
    print("ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}".format(acc_h, nmi_h, ari_h, pur_h)) if verbose else None
    return nmi_q, ari_q, acc_q, pur_q, nmi_h, ari_h, acc_h, pur_h
...
def benchmark_2024_INFU_MAGA(dataset_name="Fashion",
                             batch_size=256,
                             temperature_f=0.5,
    ...
    model = Network(view, dims, feature_dim, gcn_dim, class_num, neighbor_num).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    criterion = Loss(batch_size, class_num, temperature_f, lambda1, lambda2, eta, device)
    ...
    for epoch in range(mse_epochs):
        ...
        print("Epoch: {} Total Loss: {:.4f}".format(epoch + 1, np.mean(tot_loss_list))) if verbose else None
    ...
    ## 3.2 Contrastive Learning Training
    for epoch in range(con_epochs):
        ...
        for batch_idx, (xs, labels, _) in enumerate(data_loader):
            ...
            for v in range(view):
                ...
                loss_list.append(criterion.forword_debiased_instance(h_fusion, hs[v], y_pred_km, neg_size=neg_size)) # debiased instance loss
        ...
        print("Epoch: {} Total Loss: {:.4f}".format(epoch + 1, np.mean(total_loss_list))) if verbose else None
    
    ## 4. Evaluate the model.
    nmi_q, ari_q, acc_q, pur_q, nmi_h, ari_h, acc_h, pur_h = validation(model, dataset, view, data_size, class_num, verbose=verbose)
    return nmi_h, ari_h, acc_h, pur_h
...
if __name__ == '__main__':
    ...
    nmi, ari, acc, pur = benchmark_2024_INFU_MAGA(
