import os, sys, random, copy, math, argparse, numpy as np
import torch
...
def validation(model, dataset, view, data_size, class_num, verbose=False):
    ...
    y_pred = KMeans(n_clusters=class_num, n_init=100).fit_predict(fusion_H.cpu().numpy())
...
def benchmark_2025_ICCV_RML(dataset_name="BDGP",
                            batch_size=256,
                            temperature_f=0.5,
    ...
    model = RML(view, dims, feature_dim, high_feature_dim, class_num, multi_blocks=1, multi_heads=1).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    criterion = Loss(batch_size, class_num, temperature_f, device)
    
    ## 3. Train the model.
    model.train()
    for epoch in range(con_iterations):
        ...
        loss = loss_contrastive_1 + loss_contrastive_2
        loss.backward() 
        optimizer.step()
        print("Epoch: {} Total Loss: {:.4f}".format(epoch + 1, loss.item())) if verbose and epoch % 100 == 0 else None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RML")
    ...
    if args.dataset == "BDGP": args.miss_rate = 0.25; args.noise_rate = 0.25; args.noise_std = 0.4; args.con_iterations = 1000; args.seed = 0
    if args.dataset == "Cora": args.miss_rate = 0.25; args.noise_rate = 0.25; args.noise_std = 0.4; args.con_iterations = 500; args.seed = 9
    if args.dataset == "DHA": args.miss_rate = 0.25; args.noise_rate = 0.25; args.noise_std = 0.4; args.con_iterations = 800; args.seed = 4
    if args.dataset == "WebKB": args.miss_rate = 0.75; args.noise_rate = 0.75; args.noise_std = 0.4; args.con_iterations = 400; args.seed = 1
    if args.dataset == "NGs": args.miss_rate = 0.50; args.noise_rate = 0.50; args.noise_std = 0.4; args.con_iterations = 400; args.seed = 1
    if args.dataset == "VOC": args.miss_rate = 0.25; args.noise_rate = 0.25; args.noise_std = 0.4; args.con_iterations = 500; args.seed = 1
    if args.dataset == "YoutubeVideo": args.miss_rate = 0.25; args.noise_rate = 0.25; args.noise_std = 0.4; args.con_iterations = 200000; args.seed = 0
    if args.dataset == "Prokaryotic": args.miss_rate = 0.50; args.noise_rate = 0.50; args.noise_std = 0.4; args.con_iterations = 1000; args.seed = 9
    if args.dataset == "Cifar100": args.miss_rate = 0.25; args.noise_rate = 0.25; args.noise_std = 0.4; args.con_iterations = 10000; args.seed = 4
    nmi, ari, acc, pur = benchmark_2025_ICCV_RML(
        dataset_name=args.dataset,
        batch_size=args.batch_size,
        temperature_f=args.temperature_f,
        learning_rate=args.learning_rate,
        weight_decay=args.weight_decay,
        con_iterations=args.con_iterations,
