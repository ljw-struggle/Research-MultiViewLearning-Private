...
def validation(model, dataset, view, data_size, class_num, verbose=False):
    ...
    preds = kmeans.fit_predict(H)
    nmi, ari, acc, pur = evaluate(y, preds)
    print("Clustering on latent H (k-means):") if verbose else None
    print("ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}".format(acc, nmi, ari, pur)) if verbose else None
    return nmi, ari, acc, pur

def benchmark_2024_TMM_SCMVC(dataset_name, batch_size, learning_rate, weight_decay, pre_epochs, con_epochs, feature_dim, high_feature_dim, temperature, seed):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
if __name__ == "__main__":
    ...
    nmi, ari, acc, pur = benchmark_2024_TMM_SCMVC(
