...
if __name__ == "__main__":
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    dataset_name, batch_size, num_epochs, eval_interval, clip_norm = args.dataset, args.batch_size, args.num_epochs, args.eval_interval, args.clip_norm
    lr, betas = args.lr, args.betas
    ...
    projector_hidden_dim_list, projector_activation, projector_use_bn, projector_use_bias = args.projector_hidden_dim_list, args.projector_activation, args.projector_use_bn, args.projector_use_bias
    ddc_n_hidden, ddc_use_bn = args.ddc_n_hidden, args.ddc_use_bn
    loss_rel_sigma, loss_tau, loss_delta, loss_negative_samples_ratio, loss_adaptive_contrastive_weight, seed = args.loss_rel_sigma, args.loss_tau, args.loss_delta, args.loss_negative_samples_ratio, args.loss_adaptive_contrastive_weight, args.seed
    if dataset_name == 'blobs_overlap_5':
        backbone_hidden_dim_list = [32, 32]; backbone_output_dim = 32; lr = 1e-3;
    if dataset_name == 'blobs_overlap':
        backbone_hidden_dim_list = [32, 32]; backbone_output_dim = 32; lr = 1e-3;
    if dataset_name == 'rgbd':
        backbone_hidden_dim_list = [512, 512]; backbone_output_dim = 256; lr = 1e-3
    if dataset_name == 'voc':
        backbone_hidden_dim_list = [512, 512]; backbone_output_dim = 256; lr = 1e-3
    if dataset_name == 'ccv':
        loss_delta = 20.0
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % args.eval_interval == 0:
            acc, nmi, pur, ari = validation(model, dataset, n_views, data_size, n_clusters)
            print(f"  Evaluation - NMI: {nmi:.4f}, ARI: {ari:.4f}, ACC: {acc:.4f}, PUR: {pur:.4f}")
