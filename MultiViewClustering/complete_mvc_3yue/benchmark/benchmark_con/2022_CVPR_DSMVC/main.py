...
class Loss(nn.Module): # Adopted from https://github.com/DanielTrosten/mvc
    ...
    @staticmethod
    def cdist_squared(X, Y): # L2 distance squared, shape: (batch_size, feature_dim) @ (batch_size, feature_dim).T = (batch_size, batch_size)
...
def valid(model, dataset, view, data_size, class_num):
    model.eval()
...
if __name__ == '__main__':
    ...
    for t in range(20):
        ...
        model = DSMVC(args.view_num, dims, args.feature_dim, class_num).to(device)
        ...
        for epoch in range(args.epochs):
            if (epoch//20) % 2 == 0: # Update theta parameters
                ...
                print('Epoch {} Theta Loss: {:.4f}'.format(epoch, np.mean(theta_loss_list)))
            if (epoch//20) % 2 == 1: # Update lambda parameters
                ...
                print('Epoch {} Lambda Loss: {:.4f}'.format(epoch, np.mean(lambda_loss_list)))
        acc, nmi, pur, ari = valid(model, dataset, view, data_size, class_num)
        print('Final Results: ACC = {:.4f} NMI = {:.4f} PUR = {:.4f} ARI = {:.4f}'.format(acc, nmi, pur, ari))
