...
class DeepCCA(nn.Module):
    def __init__(self, dim_list_1, dim_list_2):
        super().__init__()
        self.model_1 = MLP(dim_list_1)
        self.model_2 = MLP(dim_list_2)
        
    def forward(self, x_1, x_2):
        z_1 = self.model_1(x_1)
        z_2 = self.model_2(x_2)
        return z_1, z_2
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DCCA')
    ...
    model = DeepCCA([dims[0], 128, 64, 32, args.outdim_size], [dims[1], 128, 64, 32, args.outdim_size]).to(device)
    criterion = DCCALoss(outdim_size=args.outdim_size, use_all_singular_values=False)
    ...
    for epoch in range(args.num_epochs):
        ...
        print('Training Epoch {}'.format(epoch), 'Loss:{:.6f}'.format(np.mean(losses)))
