...
def benchmark_2023_MM_DealMVC(dataset_name, batch_size, temperature_f, temperature_l, learning_rate, weight_decay, mse_epochs, con_epochs, tune_epochs, feature_dim, high_feature_dim, seed, threshold, num_heads, hidden_dim, dropout_rate, ffn_hidden_dim):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for epoch in range(con_epochs):
        ...
        for batch_idx, (xs, _, _) in enumerate(dataloader):
            ...
            optimizer.step()
            tot_loss += loss.item()
        # print('Contrastive Learning Epoch {}'.format(epoch), 'Loss:{:.6f}'.format(tot_loss / len(dataloader)))
    ...
    acc, nmi, pur, ari = validation(model, dataset, view, data_size, class_num, eval_multi_view=False)
...
if __name__ == "__main__":
    ...
    threshold = args.threshold; num_heads = args.num_heads; hidden_dim = args.hidden_dim; dropout_rate = args.dropout_rate; ffn_hidden_dim = args.ffn_hidden_dim;
    
    acc, nmi, pur, ari = benchmark_2023_MM_DealMVC(dataset_name, batch_size, temperature_f, temperature_l, learning_rate, weight_decay, mse_epochs, con_epochs, tune_epochs, feature_dim, high_feature_dim, seed, threshold, num_heads, hidden_dim, dropout_rate, ffn_hidden_dim)
