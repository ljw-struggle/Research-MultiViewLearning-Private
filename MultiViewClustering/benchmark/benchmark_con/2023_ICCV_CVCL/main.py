import os, random, argparse, numpy as np, scipy.io as sio
import torch
...
def get_config(args):
    if args.dataset == "MSRCv1": # db checked 92.86
        dataset = "MSRCv1"; learning_rate = 0.0005; weight_decay = 0.; batch_size = 35; seed = 10; mse_epochs = 200; con_epochs = 400; normalized = False; temperature_l = 1.0;
        ...
        dataset = "MNIST-USPS"; learning_rate = 0.0001; weight_decay = 0.; batch_size = 50; seed = 10; mse_epochs = 200; con_epochs = 200; normalized = False; temperature_l = 1.0;
        ...
        dataset = "COIL20"; learning_rate = 0.0005; weight_decay = 0.; batch_size = 180; seed = 50; mse_epochs = 200; con_epochs = 400; normalized = False; temperature_l = 1.0;
        ...
        dataset = "scene"; learning_rate = 0.0005; weight_decay = 0.; batch_size = 69; seed = 10; mse_epochs = 200; con_epochs = 100; normalized = False; temperature_l = 1.0;
        ...
        dataset = "hand"; learning_rate = 0.0001; weight_decay = 0.; batch_size = 200; seed = 50; mse_epochs = 200; con_epochs = 200; normalized = True; temperature_l = 1.0;
        ...
        dataset = "Fashion"; learning_rate = 0.0005; weight_decay = 0.; batch_size = 100; seed = 20; mse_epochs = 200; con_epochs = 100; normalized = True; temperature_l = 0.5;
        dim_high_feature = 2000; dim_low_feature = 500; dims = [256, 512]; alpha = 0.005; beta = 0.005;
    elif args.dataset == "BDGP": # db checked 99.2
        dataset = "BDGP"; learning_rate = 0.0001; weight_decay = 0.; batch_size = 250; seed = 10; mse_epochs = 200; con_epochs = 100; normalized = True; temperature_l = 1.0;
        dim_high_feature = 2000; dim_low_feature = 1024; dims = [256, 512]; alpha = 0.01; beta = 0.01;
    return dataset, learning_rate, weight_decay, batch_size, seed, mse_epochs, con_epochs, normalized, temperature_l, dim_high_feature, dim_low_feature, dims, alpha, beta
...
if __name__ == "__main__":
    ...
    for epoch in range(mse_epochs):
        ...
        print('Pre-training epoch {}, Loss:{:.7f}'.format(epoch, np.sum(losses)/ data_size))
    ## 3.2. Multi-view Contrastive Learning
    print("Multi-view Contrastive Learning...")
    for epoch in range(con_epochs):
        ...
        for batch_idx, (x_list, _, _) in enumerate(dataloader):
            ...
            optimizer.step()
            losses.append(loss.item())
        print('Contrastive_train epoch {}, Loss:{:.7f}'.format(epoch, np.sum(losses)/ data_size))
