...
def validation(model, dataset, view, data_size, class_num, eval_h=False, eval_q=False):
    ...
    if eval_h == True:
        ...
        print('\nEvaluation on feature vectors: ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}'.format(acc, nmi, ari, pur))
    if eval_q == True:
        ...
        print('\nEvaluation on cluster assignments: ACC = {:.4f}; weighted Precision = {:.4f}; weighted F1-score = {:.4f}; weighted Recall = {:.4f}'.format(accuracy, precision, f1_score, recall))
...
if __name__ == '__main__':
    ...
    validation(model, dataset, view, data_size, class_num, eval_h=True, eval_q=False)
