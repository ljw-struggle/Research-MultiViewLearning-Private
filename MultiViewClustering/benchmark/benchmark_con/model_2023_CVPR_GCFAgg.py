...
def validation(model, dataset, view, data_size, class_num, verbose=False):
    ...
    print("Clustering on latent z_concat (k-means):") if verbose else None
    print("ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}".format(acc, nmi, ari, pur)) if verbose else None
    return nmi, ari, acc, pur

def benchmark_2023_CVPR_GCFAgg(dataset_name="Hdigit",
                               batch_size=256,
                               temperature_f=0.5,
    ...
    for epoch in range(mse_epochs):
        ...
        for xs, _, _ in dataloader:
            ...
            optimizer.step()
            tot_loss += loss.item()
        print("Pre-training Epoch: {} Total Loss: {:.4f}".format(epoch + 1, tot_loss / len(dataloader))) if verbose else None
    
    ## 3.2. Fine-tuning with Contrastive Learning
    for epoch in range(tune_epochs):
        ...
        for xs, _, _ in dataloader:
            ...
            optimizer.step()
            tot_loss += loss.item()
        print("Fine-tuning Epoch: {} Total Loss: {:.4f}".format(epoch + 1, tot_loss / len(dataloader))) if verbose else None
...
if __name__ == "__main__":
    ...
    if args.dataset in tune_epochs_dict: args.tune_epochs = tune_epochs_dict[args.dataset]
    if args.dataset in seed_dict: args.seed = seed_dict[args.dataset]

    nmi, ari, acc, pur = benchmark_2023_CVPR_GCFAgg(
