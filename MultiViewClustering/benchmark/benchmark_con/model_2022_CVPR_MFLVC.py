...
def get_default_config(dataset_name):
    match dataset_name:
        case 'MNIST-USPS':
            dataset_name = 'MNIST-USPS'; batch_size = 256; temperature_f = 0.5; temperature_l = 1.0; learning_rate = 0.0003; weight_decay = 0.; mse_epochs = 200; con_epochs = 50; tune_epochs = 50; feature_dim = 512; high_feature_dim = 128; seed = 10;
        case 'BDGP':
            dataset_name = 'BDGP'; batch_size = 256; temperature_f = 0.5; temperature_l = 1.0; learning_rate = 0.0003; weight_decay = 0.; mse_epochs = 200; con_epochs = 50; tune_epochs = 50; feature_dim = 512; high_feature_dim = 128; seed = 10;
        case _:
            raise Exception(f"Dataset '{dataset_name}' is not supported")
    return dataset_name, batch_size, temperature_f, temperature_l, learning_rate, weight_decay, mse_epochs, con_epochs, tune_epochs, feature_dim, high_feature_dim, seed

def benchmark_2022_CVPR_MFLVC(dataset_name, batch_size, temperature_f, temperature_l, learning_rate, weight_decay, mse_epochs, con_epochs, tune_epochs, feature_dim, high_feature_dim, seed):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    acc, nmi, pur, ari = validation(model, dataset, view, data_size, class_num)
    return acc, nmi, pur, ari
...
if __name__ == "__main__":
    ...
    acc, nmi, pur, ari = benchmark_2022_CVPR_MFLVC(dataset_name, batch_size, temperature_f, temperature_l, learning_rate, weight_decay, mse_epochs, con_epochs, tune_epochs, feature_dim, high_feature_dim, seed)
