...
class SDMVC(nn.Module): # Self-supervised Deep Multi-view Clustering (SDMVC)
    def __init__(self, view=2, input_dims=[1750, 79], feature_dim=10, class_num=10, alpha=1.0):
        super(SDMVC, self).__init__()
        self.view = view
...
def validation(model, data, label, view, class_num, verbose=False):
    ...
    q_list = [q.detach().cpu().numpy() for q in q_list] # shape: view * [data_size, class_num]
    q_global = sum(q_list) / view # shape: (data_size, class_num)
...
def benchmark_2022_TKDE_SDMVC(dataset_name='BDGP', batch_size=256, pretrain_learning_rate=0.001, pretrain_epochs=500, dec_learning_rate=0.001, maxiter=30000, update_interval=1000, lambda_mse=1.0, lambda_clustering=0.1, alignment_ratio_threshold=0.90, patience=1, verbose=False, random_state=42):
    # SDMVC: every view perform deep embedded clustering globally.
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    model = SDMVC(view=view, input_dims=dims, feature_dim=10, class_num=class_num).to(device)
    ...
    for ite in range(int(maxiter)):
        if ite % update_interval == 0:
            ...
            q_list = [q.detach().cpu().numpy() for q in q_list] # shape: view * [data_size, class_num]
            q_global = sum(q_list) / view # shape: (data_size, class_num)
            y_pred_list = [np.argmax(q, axis=1) for q in q_list] # shape: view * [data_size,]
            y_pred_global = np.argmax(q_global, axis=1) # shape: (data_size,)
            alignment_ratio = np.sum(np.all(np.stack(y_pred_list) == y_pred_global, axis=0)) / data_size # shape: scalar
            encoded_list = [encoded.detach().cpu().numpy() for encoded in encoded_list] # shape: view * [data_size, feature_dim]
            encoded_global = np.hstack(encoded_list) # shape: (data_size, sum(feature_dim))
            kmeans = KMeans(n_clusters=class_num, n_init=100)
            kmeans.fit_predict(encoded_global)
            q_global = model.cluster_assignment_distribution(torch.tensor(encoded_global, dtype=torch.float32).to(device), torch.tensor(kmeans.cluster_centers_, dtype=torch.float32).to(device)) # shape: (data_size, class_num)
            p_global = model.target_distribution(q_global).detach() # shape: (data_size, class_num)
            nmi, ari, acc, pur = evaluate(label.cpu().detach().numpy(), y_pred_global)
            print("Iteration: {} alignment_ratio: {:.4f}; ACC = {:.4f}; NMI = {:.4f}; ARI = {:.4f}; PUR = {:.4f}".format(ite, alignment_ratio, acc, nmi, ari, pur)) if verbose else None
            alignment_times = 0 if alignment_ratio <= alignment_ratio_threshold else alignment_times + 1
            if alignment_times >= patience:
                print('Converged, stopping training...') if verbose else None; 
                break # stop the training loop if the alignment times is greater than the patience
        model.train()
        ...
        # IDEA: q_batch = sum(q_views_list) / view, shape: [batch_size, class_num]
        loss = lambda_clustering * sum([criterion_kl(torch.log(q_batch[v]), p_global[idx]) for v in range(view)]) + lambda_mse * sum([criterion_mse(decoded_batch[v], data_batch[v]) for v in range(view)])
        loss.backward()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SDMVC")
    ...
    parser.add_argument("--alignment_ratio_threshold", default=0.90, type=float)
    parser.add_argument("--patience", default=1, type=int)
    parser.add_argument("--seed", default=42, type=int)
    args = parser.parse_args()
    
    nmi, ari, acc, pur = benchmark_2022_TKDE_SDMVC(
