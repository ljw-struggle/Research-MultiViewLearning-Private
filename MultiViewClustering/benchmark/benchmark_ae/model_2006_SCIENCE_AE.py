...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', default='./data/data_bulk_multiomics/LGG/', help='the data dir [default: ./data/data_bulk_multiomics/BRCA/]')
    parser.add_argument('--output_dir', default='./result/data_bulk_multiomics/AE/LGG/', help='the output dir [default: ./result/data_bulk_multiomics/AE/BRCA/]')
    ...
    for t in range(args.times):
        ...
        model = AE(embed_dim=args.latent_dim, num_views=data_views, feature_dims=data_features, hidden_dims=[512, 512, 512]).to(device)
        ...
        model.eval()
        _, embedding = model.forward(data)
        from _utils import clustering_acc, normalized_mutual_info_score, silhouette_score, adjusted_rand_score
        kmeans = KMeans(n_clusters=data_categories, n_init=20, random_state=0)
        preds = kmeans.fit_predict(embedding.detach().cpu().numpy())
        acc_val = clustering_acc(label, preds)
        nmi_val = normalized_mutual_info_score(label, preds)
        asw_val = silhouette_score(embedding.detach().cpu().numpy(), preds)
