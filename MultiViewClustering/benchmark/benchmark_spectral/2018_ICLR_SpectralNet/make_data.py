...
def save_hdf5(X, y, name): # from dec (https://github.com/piiswrong/dec/tree/master/dec)
    import h5py
    with h5py.File('./{}.h5'.format(name), 'w') as f:
        f['data'] = X
        f['labels'] = y

def make_reuters_data(): # from dec (https://github.com/piiswrong/dec/tree/master/dec)
    np.random.seed(1234)
    random.seed(1234)
