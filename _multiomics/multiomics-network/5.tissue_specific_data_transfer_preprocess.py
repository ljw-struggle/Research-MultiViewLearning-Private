...
file_handler = logging.FileHandler(filename='tissue_specific_data/ProcessedData/expression_data_transfer.log', mode='a+')
...
def preprocess_tissue_specific_data_new(expression_file, regulation_file, balance, save_path):
    ...
    # sc.pp.filter_genes(adata, min_cells=100, inplace=True) # filter genes expressed in less than 100 cells
    # sc.pp.filter_cells(adata, min_genes=100, inplace=True) # filter cells expressing in less than 100 genes
    # cell_index = np.random.choice(adata.shape[0], 1000, replace=False) # randomly select 1000 cells
    # adata = adata[cell_index, :] # filter cells
    ...
    gene_list = regulation_data['DORC'].str.upper().tolist()
    ...
    expression_data = expression_data[expression_data.index.isin(gene_list)]
    species = 'HomoSapiens' if 'Human' in expression_file else 'MusMusculus'
    tf_list = pd.read_csv('./species_specific_data/ProcessedData/{}/protein.seq'.format(species), sep='\t', header=None)[0].str.upper().tolist()
    target_list = pd.read_csv('./species_specific_data/ProcessedData/{}/promoter.seq'.format(species), sep='\t', header=None)[0].str.upper().tolist()
    ...
    expression_data = expression_data[expression_data.index.isin(target_list)]    
    TF = regulation_data['TF'].unique()
    GENE = expression_data.index.tolist()
    ...
    if balance == 'False':
        train_num_pos, train_num_neg, val_num_pos, val_num_neg, test_num_pos, test_num_neg = train_test_val_split_hard(regulation_file, expression_file, save_path, trick=True)
    if balance == 'True':
        train_num_pos, train_num_neg, val_num_pos, val_num_neg, test_num_pos, test_num_neg = train_test_val_split_balance(regulation_file, expression_file, save_path, trick=True)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Tissue specific data preprocess.')
    ...
    # Preprocess data.
    specific_expression_file, specific_regulation_file = preprocess_tissue_specific_data_new(args.exp_file, args.network_file, balance=args.balance, save_path=args.out_prefix)
