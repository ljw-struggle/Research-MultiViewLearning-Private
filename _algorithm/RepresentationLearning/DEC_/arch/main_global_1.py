...
def train_dec(model, data, y_true=None, lr=0.01, batch_size=256, maxiter=20000, update_interval=140, tol=1e-3):
    ...
    acc_val = clustering_acc(y_true, preds)
    nmi_val = normalized_mutual_info_score(y_true, preds)
    ari_val = adjusted_rand_score(y_true, preds)
    print(f'Initial clustering completed; ACC: {acc_val:.4f}, NMI: {nmi_val:.4f}, ARI: {ari_val:.4f}')
    ...
    for ite in range(int(maxiter)):
        if ite % update_interval == 0:
            ...
            print(f'[Iter {ite}] loss: {np.mean(per_epoch_losses):.4f}') if ite > 0 else None
            per_epoch_losses = []
        model.train()
        optimizer.zero_grad()
        idx = index_array[(ite * batch_size) % len(data):(ite * batch_size + batch_size) % len(data) if (ite * batch_size + batch_size) < len(data) else len(data)] # problem: when start < len(data), end > len(data), so we need to use modulo operation to ensure the index is within the range of the data
...
if __name__ == '__main__':
    ...
    pretrain_autoencoder(model.auto_encoder, data, epochs=50, lr=1.0)
